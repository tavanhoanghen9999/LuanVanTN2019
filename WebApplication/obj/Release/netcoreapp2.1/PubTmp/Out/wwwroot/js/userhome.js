...
//get the describe product (sản phẩm mới)
getDesPr();
function getDesPr() {
    $.ajax({
        type: "get",
            ...
            if (data.success) {
                for (var i in data.data) {
                    ...
                    $("#f-despr").append('<a href="/product/product?prid=' + item.product.prid + '"><div class="k bd-id-new">' +
                        ...
                        '<span class="k t t-price-it">' + formatNumber(item.product.price) + ' đ</span>' +
                        '<span class="k t t-inf-it" style="color:#bb3030;">Điểm nổi bật: ' + describes + '</span>' +
                        '<span class="k t t-name-it-new">Thông số kỹ thuật: ' + item.product.note + '</span>' +
                        '</div>' +
                        '</div>' +
                        '</div></a>');
                }
